...
static int dom_read_map_start(uint32_t len, void *data) {
	...
	v->map.len = len;
	v->map.items = NULL;
	struct rmsgpack_dom_pair *items = calloc(len, sizeof(struct rmsgpack_dom_pair));
	if (!items) {
		return -ENOMEM;
	}
	v->map.items = items;
...
static int dom_read_array_start(uint32_t len, void *data) {
	...
	v->array.len = len;
	v->array.items = NULL;
	struct rmsgpack_dom_value *items = calloc(len, sizeof(struct rmsgpack_dom_pair));
	if (!items) {
		return -ENOMEM;
	}
	v->array.items = items;
...
void rmsgpack_dom_value_print(struct rmsgpack_dom_value *obj) {
	...
	case RDT_BINARY:
		printf("\"");
		for (int i = 0; i < obj->binary.len; i++) {
